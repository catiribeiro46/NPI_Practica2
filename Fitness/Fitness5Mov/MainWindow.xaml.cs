...
// Ana Catarina Ribeiro Marinho
// Carla Marisa Lobo Simões
// Trabajo práctico 2 : Kinect Ejercício de Fitness
// Fecha:   27/11/2014
...
public partial class MainWindow : Window
{
    ...
    /// <summary>
    ///Object to control the correct movement detected
    /// </summary>
    private fitness mov;
    ...
    //Draw rectangle with the the projected skeleton points
    private void DrawRect(Skeleton skeleton, SkeletonPoint p, Pen color, DrawingContext drawingContext)
    {
        // Render Torso
        this.DrawBone(skeleton, drawingContext, JointType.Head, JointType.ShoulderCenter);
        this.DrawBone(skeleton, drawingContext, JointType.ShoulderCenter, JointType.ShoulderLeft);
        ...
        foreach (Joint joint in skeleton.Joints)
        {
            ...
            if (drawBrush != null)
            {
                drawingContext.DrawRectangle(drawBrush, color, new Rect(this.SkeletonPointToScreen(p).X, this.SkeletonPointToScreen(p).Y, 20, 20));
                //drawingContext.DrawRectangle(drawBrush, color, new Rect(p.X, p.Y, 20, 20));
            }
        }
    }
    ...
    {
        if (button1.AreAnyTouchesCaptured)
        {
            InitializeComponent();
            mov = new fitness();
        }
