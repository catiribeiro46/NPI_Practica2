...
namespace Microsoft.Samples.Kinect.SkeletonBasics
{
    class Movimiento4
    {
        ...
        public bool AreFeetSeparate(Skeleton received)
        {
            ...
            foreach (Joint joint in received.Joints)
            {
                ...
                {//first verify if the body is alignet and arms are in a relaxed position
                    ...
                    double KneeRPosY = received.Joints[JointType.KneeLeft].Position.Y;
                    ...
                    pieIzq = (float)(DistSeparateFoot + DistErrorL);
                    pieDer = (float)(DistSeparateFoot - DistErrorL);
                    //its just verifying the distance between the two ankles
                    //need to insert new variables like KneeLPosY equals KneeRPosY with error 0.05
                    if (Math.Abs(AnkRPosX - AnkLPosX) <= Math.Abs(DistSeparateFoot + DistErrorL) && Math.Abs(AnkRPosX - AnkLPosX) >= Math.Abs((DistSeparateFoot) - DistErrorL) && Math.Abs(KneeLPosY - KneeRPosY) <= 0.02 && Math.Abs(AnkLPosY - AnkRPosY) <= 0.02)
                    {
                        pieIzq = (float)AnkLPosX;
                        pieDer = (float)AnkRPosX;
                        alturaPierna = (float)distHiptoAnkleR;
                        return true;
                    }
                    else return false;
        ...
        {
            if (IsAlignedBodyAndArms(received) && AreFeetSeparate(received)) return true;
            else return false;
        }
